// Brick generator by Chris Finke (cfinke)
// https://www.thingiverse.com/thing:615256/
// Original licensed under Creative Commons - Attribution - Share Alike license
// LEGO, the LEGO logo, the Brick, DUPLO, and MINDSTORMS are trademarks of the LEGO Group. 2012 The LEGO Group.

// It is suggested to print tiles upside down

LEGO_BRICK_L = 8;
LEGO_BRICK_W = 8;
LEGO_BRICK_H = 9.6;

DUPLO_BRICK_L = LEGO_BRICK_L*2;
DUPLO_BRICK_W = LEGO_BRICK_W*2;
DUPLO_BRICK_H = LEGO_BRICK_H*2;
...
module LEGO(
  ...
  // Height of the block relative to a standard brick: A ratio of "1"
  // is a standard LEGO brick height; a ratio of "1/3" is a standard
  // LEGO plate height; "1/2" is a standard DUPLO plate.
  height = 1, // [1/3:LEGO Plate, .5:Duplo Plate, 1:LEGO Brick, 2:2 DUPLO Brick, 3:3, 4:4]

  type="brick", // [brick:Brick, tile:Tile, wing:Wing, slope:Slope, curve:Curve, baseplate:Baseplate]

  brand="lego", // [lego:LEGO, duplo:DUPLO]

  stud_type="solid", // [solid:Solid, hollow:Hollow]

  // Technic holes, yes or no
  horizontal_holes=false,

  // Vertical cross-shaped axle holes, yes or no
  vertical_axle_holes=false,


  /** [Wings] **/

  // What type of wing? Full is suitable for the front of a plane,
  // left/right are for the left/right of a plane.
  wing_type="full", // [full:Full, left:Left, right:Right]

  // The number of studs across the end of the wing. If block_width is
  // odd, this needs to be odd, and the same for even.
  wing_end_width=2,
  ...
  // How much vertical height should be left at the end of the slope?
  // e.g, a value of zero means the slope reaches the bottom of the
  // block. A value of 1 means that for a block with height 2, the
  // slope reaches halfway down the block.
  slope_end_height=0,
  ...
  // Should the curve be convex or concave? // [concave:Concave, convex:Convex]
  curve_type="concave",

  // How much vertical height should be left at the end of the curve?
  // e.g, a value of zero means the curve reaches the bottom of the
  // block. A value of 1 means that for a block with height 2, the
  // curve reaches halfway down the block.
  curve_end_height=0,


  /** [Baseplates] **/

  // How wide should the roadway be (in studs)? A roadway is a smooth
  // (non-studded) section of a baseplate.
  roadway_width=0,

  // How long should the roadway be?
  roadway_length=0,

  // Where should the roadway start (x-value)? A value of zero puts
  // the roadway at the far left side of the plate.
  roadway_x=0,

  // Where should the roadway start (y-value)? A value of zero puts
  // the roadway at the front of the plate.
  roadway_y=0,
  ...
  // Should extra reinforcement be included to make printing on an FDM
  // printer easier? Automatically sets for Duplo bricks. Otherwise
  // recommended for block heights less than 1.
  reinforcement=false,

  // If your printer prints the blocks correctly except for the stud
  // diameter, use this variable to resize just the studs for your
  // printer. A value of 1.05 will print the studs 105% wider than
  // standard.
  stud_rescale=1,
  ...
  real_wing_base_length =
    min(real_length-1, max(1, wing_base_length)) + 1; // +1 because the angle starts before the last stud.
  ...
  real_reinforcement = (reinforcement || brand=="duplo")
    && type != "baseplate" && !dual_sided; // tjkopena: Removed clause 'type != "tile"'
  ...
  total_posts_width = (post_diameter * (real_width - 1)) +
    ((real_width - 2) * (stud_spacing - post_diameter));
  total_posts_length = (post_diameter * (real_length - 1)) +
    ((real_length - 2) * (stud_spacing - post_diameter));

  total_axles_width = (axle_diameter * (real_width - 1)) +
    ((real_width - 2) * (stud_spacing - axle_diameter));
  total_axles_length = (axle_diameter * (real_length - 1)) +
    ((real_length - 2) * (stud_spacing - axle_diameter));

  total_pins_width = (pin_diameter * (real_width - 1)) +
    max(0, ((real_width - 2) * (stud_spacing - pin_diameter)));
  total_pins_length = (pin_diameter * (real_length - 1)) +
    max(0, ((real_length - 2) * (stud_spacing - pin_diameter)));
  ...
  wing_slope = (wing_type == "full" ?
   ...
   translate([-overall_length/2, -overall_width/2, 0]) // Comment to position at 0,0,0 instead of centered on X and Y.
    ...
    // Modification from EduardoFoltran in comments
    module stud() {
...
/* Original study function w/out chamfer
    module stud() {
        difference() {
...
module LEGO_uncenter(width, length, height) {
    // stud_spacing = 8
    // wall_play = 0.1
    translate([((8 * length) / 2) - 0.1, ((8 * width) / 2) - 0.1, height ? ((8 * height) / 2) - 0.1 : 0]) children();
}

module LEGO_place(x, y, z=0) {
    translate([8 * y, 8 * x, z * 9.6]) children();
}

function LEGO_mm(studs, brand="lego") =
  let (stud_spacing = (brand == "lego" ? 8 : 8 * 2),
       overall_width = (studs * stud_spacing) - (2 * wall_play)
       ) overall_width;
